...
// Simply updating code from array_linked_lists.c and selection_sort_arrays.c
...
// Now lets us write the selection sort algorithm for the linked list
void selectionSort(Node* head)
{
    ...
    while(temp != NULL)
    {
        // Original assumption is that the first element is the minimum
        min = temp;
        // Setting the value of temp2 to the next element so that we can compare it with the minimum
        // and change pointers if required
        temp2 = temp->next;

        // Finding the minimum element in the linked list
        while(temp2 != NULL)
        {
            // If the value of the next element is less than the value of the minimum element
            // then we change the value of the minimum element
        ...
        // Now we swap the order of the minimum element and the current element
        tempdata = temp->data;
        temp->data = min->data;
        min->data = tempdata;
        temp = temp->next;
